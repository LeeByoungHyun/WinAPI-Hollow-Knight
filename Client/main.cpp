...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,         /* 실행 된 프로세스의 시작 주소 */
                     _In_opt_ HINSTANCE hPrevInstance,  /* 이전에 실행 된 프로세스의 시작 주소 */ 
                                                        /* 과거의 잔재. 지금은 여러개의 프로세스를 메모리에 올려도 모두 주소값이 같다(가상메모리) */
                     ...
                     _In_ int       nCmdShow)
{
    ...
    while (true)
    {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) // 메세지가 있으면 if문 실행, 없으면 else문 실행
        {
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:  // 무효화 영역(Invaildate)이 발생한 경우
                    // 창을 최소화했다가 다시 활성화 할 때 발생
        {
            ...
            HBRUSH hBlueBrush = (HBRUSH)GetStockObject(DC_BRUSH);
            ...
            */
            ...
            // 
            // 윈도우 좌표
            // 윈도우 좌표 (0, 0)은 화면 왼쪽 위
            // 단위는 픽셀 ex) (100, 200) : 왼쪽 위에서 아래로 100픽셀, 오른쪽으로 200픽셀
